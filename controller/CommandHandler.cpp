...
CommandHandler::CommandHandler(MavlinkSystem* mavlink, TelemetryCache* telemetryCache, bool simulatorMode, const std::string& simulatorPreset, bool debugDetector, double simulatorSnrDb,
                               ...
                               const std::string& simulatorAntenna, double simulatorPriPpm,
    ...
    , _simulatorNoiseSource (simulatorNoiseSource)
    , _simulatorAntenna (simulatorAntenna)
    , _simulatorPriPpm  (simulatorPriPpm)
    , _debugDetector    (debugDetector)
    , _detectorImpulseBlankFactor (detectorImpulseBlankFactor)
{
    if (isRunningOnRPi()) {
...
void CommandHandler::_startPythonDetector(LogFileManager* logFileManager, const TunnelProtocol::TagInfo_t& tagInfo, bool secondaryChannel, bool isHFMode, double detectionMargin, double confidenceRatio, bool debugDetector, bool dumpSpectrogram, int controlPort)
{
    ...
    if (_detectorImpulseBlankFactor > 0.0) {
        commandStr += formatString(" --impulse-blank-factor %f", _detectorImpulseBlankFactor);
    }
    if (controlPort > 0) {
...
void CommandHandler::stopDetectionPipeline(uint32_t requestId)
{
    ...
    std::thread([this, inRotation, rotationOwnsProgress]() {
        ...
        // A child that survived SIGTERM may still own the SDR, so stay Stopping.
        if (allExited) {
                                        ...
                                        const TagTrackerDetectorProtocol::PulsePayload& payload)
{
    ...
    std::vector<PythonPulseInfo_t> replay;
    {
        ...
        if (_inRotation
            ...
            // Locked measurements carry the fold score of an independent
            // sighting of this candidate on this slice, 0 if only measured.
            slice.sighted           = pulseInfo.detection_status == kConfirmedDetectionStatus
                                      && pulseInfo.score_ratio > 0.0;
...
BearingCalculator CommandHandler::_bearingCalculatorFor(const std::vector<RotationSlice>& slices) const
{
    BearingCalculator calculator(AntennaPatterns::byId(_antennaId));
...
{
    std::vector<RotationSlice> tagSlices;
    for (const auto& slice : _rotationSlices) {
        if (slice.tag_id == tagId) {
            tagSlices.push_back(slice);
        }
    }
    BearingCalculator calculator = _bearingCalculatorFor(tagSlices);
...
std::string CommandHandler::startCollection(const mavlink_tunnel_t& tunnel)
{
    ...
    bool startPipeline = false;
    {
        ...
        if (result == CollectionCoordinator::Result::Duplicate && collectionInfo.antenna_id != _antennaId) {
            // A retry re-sends the same payload; a different antenna is a new
            // configuration the running collection would silently ignore.
    ...
    if (!ready || !isEstablished()) {
        ...
        // The start worker cannot be cancelled and normally ends in startFinished();
        // leaving before it does would let it publish Detecting with no collection.
        // This runs on the command thread, so the wait is bounded: a start that
        // never settles is reported and left for the operator.
        ...
        if (!startSettled) {
            logError() << "Collection cleanup: detection start did not settle; SDR stays reserved until controller restart";
            _mavlink->sendStatusText("#Detection start hung; restart controller", MAV_SEVERITY_ALERT);
        } else if (_dispatcher.detection().state() == DetectionCoordinator::State::Detecting
                   || _dispatcher.detection().state() == DetectionCoordinator::State::Stopping) {
    ...
    float revisitHeadingDeg)
{
    ...
    message.revisit_heading_deg = revisitHeadingDeg;
    {
        std::lock_guard<std::mutex> lock(_rotationMutex);
...
std::string CommandHandler::startCollectionSlice(const mavlink_tunnel_t& tunnel)
{
    ...
    bool replayComplete = false;
    {
        ...
        if (!replayComplete) {
            ...
            // Only a slice flown at the requested heading is the confirmation
            // dwell; any other slice leaves the revisit outstanding so the
            // next FINISH asks again instead of finalizing unconfirmed.
            if (_pendingRevisitHeadingDeg) {
                const float delta = std::fabs(std::remainder(sliceInfo.heading_deg - *_pendingRevisitHeadingDeg, 360.0f));
...
std::string CommandHandler::finishCollection(const mavlink_tunnel_t& tunnel)
{
    ...
    // Before tearing the detectors down (their locks and buffered slices
    // live in the detector processes), see whether the rotation needs one
    // confirmation dwell. If so the collection stays open and the GCS flies
    // that heading as an extra slice, then finishes again. A FINISH retry
    // (lost ack or status) before that slice is armed gets the same request
    // again rather than finalizing without the confirmation.
    if (finalize) {
        // A revisit asks the GCS to fly another slice, which needs live
        // detectors. While they are starting or being torn down the answer
        // would be wrong either way, so let the retry decide; if they are
        // simply not running (operator STOP, raw capture) there is nothing to
        // fly the slice with, so finalize on the slices already collected.
        ...
        if (canRevisit) {
            std::lock_guard<std::mutex> lock(_rotationMutex);
            if (_collectionCoordinator.collectionId() == finishInfo.collection_id
                && _collectionCoordinator.state() == CollectionCoordinator::State::Ready) {
                if (_pendingRevisitHeadingDeg) {
                    revisitHeading = _pendingRevisitHeadingDeg;
                    replayed = true;
                } else if (!_revisitRequested) {
                    revisitHeading = BearingCalculator::revisitHeadingFor(_bearingCalculatorFor(_rotationSlices).solve());
                    if (revisitHeading) {
                        _revisitRequested = true;
                        _pendingRevisitHeadingDeg = revisitHeading;
                    }
                }
            }
        }
    ...
    // Settle the start before finalize(): a NACK here must leave the
    // collection active so the retry can run this path again.
    ...
    // Likewise tear detection down before finalize()/cancel(), but only where
    // the command is going to be accepted — finalize of the live collection
    // between slices, cancel of the live collection in any state — so a FINISH
    // for an unknown id or a finalize mid-slice has no side effects. A child
    // that outlives the teardown deadline keeps the collection open for retry.
    {
        bool willAccept;
        {
            std::lock_guard<std::mutex> lock(_rotationMutex);
...
void CommandHandler::_sendFinishFrame(FinishOutcome& outcome, const void* payload, size_t size)
{
    ...
    auto& frame = outcome.frames.emplace_back(bytes, bytes + size);
    _mavlink->sendTunnelMessage(frame.data(), frame.size());
}
...
void CommandHandler::replayFinishOutcome(uint32_t collectionId)
{
    std::lock_guard<std::mutex> lock(_rotationMutex);
...
std::string CommandHandler::rawCapture(const mavlink_tunnel_t& tunnel)
{
    ...
    // constexpr: usable in the lambda without a capture (clang -Wunused-lambda-capture).
...
std::string CommandHandler::_simulatorCommand(uint32_t radioCenterFrequencyHz)
{
    ...
    std::string txArgs = formatString(" --tx-offset-north-m %f --tx-offset-east-m %f --antenna %s --pri-ppm %f",
                                      kSimulatorTxRangeM * std::cos(txBearingRad),
                                      kSimulatorTxRangeM * std::sin(txBearingRad),
                                      _simulatorAntenna.c_str(),
                                      _simulatorPriPpm);
    if (_simulatorNoiseSource.db > 0.0) {
        txArgs += formatString(" --noise-source-db %f --noise-source-bearing-deg %f%s",
                               _simulatorNoiseSource.db, _simulatorNoiseSource.bearingDeg,
                               _simulatorNoiseSource.impulsive ? " --noise-source-impulsive" : "");
    }
    ...
    for (const auto& tagInfo : _tagDatabase) {
        ...
        if (tagInfo.intra_pulse2_msecs != 0) {
            ...
            const uint32_t k = tagInfo.k;   // validated in _handleTag
