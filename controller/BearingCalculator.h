...
public:
    ...
    // K-pulse mean has variance proportional to noise_psd^2 / K; K is taken
    // as equal for every slice, hence w = (median noise / noise)^2,
    // dimensionless and 1 at the typical heading. A gap-clipped slice
    // averages fewer pulses and is over-weighted by K/n (#153). Slices with
    // unknown noise get 1; if no slice has a usable noise the fit is unweighted.
