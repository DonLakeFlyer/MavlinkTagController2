...
void OperationProgressReporter::_updateLocked(uint32_t step, uint32_t stepCount, const std::string& message)
{
    ...
    const uint32_t clamped = stepCount ? std::min(step, stepCount) : step;
    if (clamped == _current.step && stepCount == _current.step_count && !messageChanged) {
        return;
    }
    _current.step_count = stepCount;
    _current.step       = clamped;
    if (messageChanged) {
