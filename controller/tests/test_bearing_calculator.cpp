...
// ── Test: a noisy heading whose raw power beats the true peak must not steer the scan ─
static void testNoisyHeadingCannotCentreScan() {
    for (const AntennaPattern* pattern : {&AntennaPatterns::ra2a(), &AntennaPatterns::ra23k()}) {
        const float trueBearing = 45.0f;
        auto slices = generateSlices(trueBearing, 30.0, 0.0, 8, *pattern);
        // The back-lobe heading (225 deg) fluctuates above the 30-unit peak
        // at 100x the noise. If it centred the +/-90 deg scan the true
        // bearing would never be evaluated.
        BearingCalculator weighted(*pattern);
        weighted.setConfidenceFloor(0.0f);
        for (const auto& [hdg, power] : slices) {
            const bool noisy = hdg == 225.0f;
            weighted.addSlice(hdg, noisy ? power + 40.0 : power, 5, 0.0, 0, false, noisy ? 1e-7 : 1e-9);
        }
        auto r = weighted.solve();
        CHECK(r.size() == 1);
        assertNear(r[0].bearing_deg, trueBearing, kBearingToleranceDeg, "weighted bearing with noisy heading above peak");
        std::printf("PASS: testNoisyHeadingCannotCentreScan (%s: %.1f R²=%.3f)\n",
                    pattern->name, r[0].bearing_deg, r[0].r_squared);
    }
}
...
int main() {
    ...
    testNoisyHeadingCannotCentreScan();
