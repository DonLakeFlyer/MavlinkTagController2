...
| `test_detection_coordinator` — `test_detection_coordinator.cpp` | `DetectionCoordinator`: `Idle` → `HasTags` → `Starting` → `Detecting` → `Stopping` → `HasTags`, plus `HasTags` → `Capturing` → `HasTags`; `AlreadyStarting` / `AlreadyStopping` for in-flight retries, `NoTags`, `AlreadyDetecting`, `NotDetecting`, `StartInProgress`, `Busy`, `Capturing` / `AlreadyCapturing`; failed start returns to `HasTags`; stray `*Finished` calls ignored; heartbeat sink fires once per published change; `waitWhile` wakes on transition. |
