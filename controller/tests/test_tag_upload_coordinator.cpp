...
void testEndTagsRetransmitIsIdempotent()
{
    ...
    CHECK(upload.endTags(end(2)) == Result::CountMismatch); // same upload, wrong count is not a retry
    CHECK(upload.state() == State::HasTags);
...
void testStartTagsClearsPreviousUpload()
{
    ...
    // Oversized count is refused before anything is cleared.
    CHECK(upload.startTags(true, start(TagUploadCoordinator::kMaxTagCount + 1, kUpload + 1)) == Result::InvalidCount);
    CHECK(upload.startTags(true, start(0xFFFFFFFFu, kUpload + 1)) == Result::InvalidCount);
    CHECK(upload.state() == State::HasTags);
    CHECK(upload.tags().size() == 2);
    CHECK(upload.uploadId() == kUpload);
    CHECK(upload.startTags(true, start(TagUploadCoordinator::kMaxTagCount, kUpload + 1)) == Result::Accepted);
