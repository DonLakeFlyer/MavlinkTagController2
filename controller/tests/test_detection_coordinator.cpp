...
// Raw capture is a state: claimed synchronously, published as CAPTURE, and
// mutually exclusive with start and tag upload.
void testCapture()
{
    Harness h;
    CHECK(h.coord.requestCapture() == Result::NoTags);
    h.coord.tagsUploaded(true);
    CHECK(h.coord.requestCapture() == Result::Accepted);
    CHECK(h.coord.state() == State::Capturing);
    CHECK(h.coord.heartbeatStatus() == HEARTBEAT_STATUS_CAPTURE);
    CHECK(!h.coord.idle());
    CHECK(h.coord.requestCapture() == Result::AlreadyCapturing);
    CHECK(h.coord.requestStart() == Result::Capturing);
    CHECK(h.coord.tagsUploaded(false) == Result::Capturing);
    CHECK(h.coord.requestStop() == Result::NotDetecting);
    h.coord.captureFinished();
    CHECK(h.coord.state() == State::HasTags);
    CHECK(h.coord.heartbeatStatus() == HEARTBEAT_STATUS_HAS_TAGS);
    CHECK((h.heartbeats == std::vector<uint16_t>{HEARTBEAT_STATUS_HAS_TAGS, HEARTBEAT_STATUS_CAPTURE, HEARTBEAT_STATUS_HAS_TAGS}));

    h.coord.requestStart();
    CHECK(h.coord.requestCapture() == Result::Busy);
    h.coord.startFinished(true);
    CHECK(h.coord.requestCapture() == Result::Busy);
    h.coord.requestStop();
    CHECK(h.coord.requestCapture() == Result::Busy);
    h.coord.captureFinished();                              // stray: ignored
    CHECK(h.coord.state() == State::Stopping);
}
