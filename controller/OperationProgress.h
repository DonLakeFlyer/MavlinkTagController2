...
public:
    ...
    /// Re-sends the RUNNING frame, or the COMPLETE/FAILED frame for a few more
    /// ticks after finish(); called from the 1 Hz heartbeat so a lost frame
    /// does not strand the GCS.
...
private:
    ...
    LogFn                               _logFn;
    bool                                _running = false;
    // Heartbeat ticks the terminal frame is still re-sent after finish().
    static constexpr unsigned           kTerminalResends = 3;
    unsigned                            _terminalResends = 0;
    std::string                         _title;
