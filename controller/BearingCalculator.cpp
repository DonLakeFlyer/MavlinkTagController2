...
std::vector<double> BearingCalculator::sliceWeights(const std::vector<SliceData>& slices)
{
    ...
    // Upper median: a common scale factor, so only the logged weights see it.
...
BearingCalculator::Result BearingCalculator::_solveForTag(uint32_t tag_id, const std::vector<SliceData>& slices) const
{
    ...
    double bestEvidence = -std::numeric_limits<double>::infinity();
    ...
    for (size_t i = 0; i < slices.size(); ++i) {
        ...
        bestPower = std::max(bestPower, s.signal_power);
        // Scan centre: power in units of the heading's noise sigma, so a
        // fluctuation on a noisy heading cannot outrank the true peak.
        const double evidence = s.signal_power * std::sqrt(weights[i]);
    ...
    // The strongest detection (in noise-sigma units) must lie in the front
    // half of the pattern, so only scan φ within ±90° of it. This also breaks
    // the front/back mirror ambiguity of the symmetric pattern.
