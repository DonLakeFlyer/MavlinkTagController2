...
DetectionCoordinator::Result DetectionCoordinator::requestStart()
{
    ...
    case State::Capturing:  return Result::Capturing;
...
DetectionCoordinator::Result DetectionCoordinator::requestStop()
{
    ...
    case State::Idle:
    case State::HasTags:
    case State::Capturing:  return Result::NotDetecting;
...
DetectionCoordinator::Result DetectionCoordinator::requestCapture()
{
    std::unique_lock<std::mutex> lock(_mutex);
    switch (_state) {
    case State::Idle:       return Result::NoTags;
    case State::Capturing:  return Result::AlreadyCapturing;
    case State::Starting:
    case State::Detecting:
    case State::Stopping:   return Result::Busy;
    case State::HasTags:    break;
    }
    _transition(State::Capturing, lock);
    return Result::Accepted;
}

void DetectionCoordinator::captureFinished()
{
    std::unique_lock<std::mutex> lock(_mutex);
    if (_state != State::Capturing) {
        return;
...
uint16_t DetectionCoordinator::heartbeatStatusFor(State state)
{
    ...
    case State::Capturing:  return HEARTBEAT_STATUS_CAPTURE;
    }
