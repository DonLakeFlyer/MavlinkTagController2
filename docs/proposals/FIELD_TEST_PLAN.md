...
the site (per-heading `noise_psd` higher toward the lines with the
noise-normalised `mu` roughly constant — an elevated `mu` there means the
noise is heavy-tailed, so try `--detector-impulse-blank-factor`; unchanged
detection at the clean site); blanked fraction reported per dwell when
blanking is on.
