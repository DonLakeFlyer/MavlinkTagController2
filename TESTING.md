...
| `test_operation_progress` | `controller/tests/test_operation_progress.cpp` | `OperationProgressReporter`: begin/update/finish frame sequence, busy gate, indeterminate → counted, 1 Hz re-send only while RUNNING, message truncation, stable log line | |
