...
def report_frequency_hz(center_freq_mhz, configured_freq_hz, offset_hz):
    """Absolute Hz of a detection: the measured bin when the channel centre
    is known, so the controller can cluster hits in frequency."""
    if center_freq_mhz > 0:
        return int(round(center_freq_mhz * 1e6 + offset_hz))
    return configured_freq_hz if configured_freq_hz else int(round(offset_hz))
...
def fold_multi_hypothesis(power, hypotheses):
    ...
    # Pool once for every hypothesis; pooling costs more than the fold itself.
    pooled = _pooled_power(power, FOLD_LOCAL_RADIUS)
    for label, pulse_idx in hypotheses:
        # fold_scores shape: (n_freq, search_range)
        fold_scores = _compute_fold_scores(pooled, pulse_idx, local_radius=0)
...
def fit_lock_timing(buffered_slices, freq_axis, lock, n_ws, fs,
    ...
    # Lever arm to the farthest window that actually held data: zero-filled
    # windows were excluded above and pin nothing.
    lever_s = 0.0
    for _, _, segment_start, _, _, valid in rows:
        valid_idx = np.flatnonzero(valid)
        if valid_idx.size == 0:
            continue
        lever_s = max(lever_s,
                      abs(segment_start + valid_idx[0] * step_s - best_anchor),
                      abs(segment_start + (valid_idx[-1] + 1) * step_s - best_anchor))
...
def permutation_null_threshold(power, noise_power, pf, n_perm,
    ...
    Returns ``(inf, None, None, n_perm_used)`` when fewer than
    ``NULL_MIN_PERMUTATIONS`` permutations were usable.
    """
...
def fold_detect(power, N, pf, Fs, nfft, n_w, n_ol, samples_needed,
    ...
    Args:
        ...
        threshold_state:     dict carried across cycles by the caller. Keys:
                             ``margin_logged`` (set here); optional ``rng``
                             (np.random.Generator) used by the permutation
                             null and window resampling so a replay of the
                             same IQ reproduces the same thresholds; optional
    ...
    null_rng = threshold_state.get('rng')
    null_refined = False
    n_dropped_windows = 0
    if fixed_threshold is not None:
        ...
        if drop_windows:
            ...
            refined = resample_windows(power, drop, rng=null_rng)
...
def main():
    ...
    ap.add_argument('--center-freq', type=float, default=0.0,
                    help='Channel center frequency in MHz; pulse reports carry '
                         'center + detected offset when set')
    ...
    slog.emit(STARTUP, _startup_human,
              ...
              null_time_budget=args.null_time_budget,
              impulse_blank_factor=args.impulse_blank_factor,
    ...
    threshold_state = {'rng': np.random.default_rng(args.tag_id)}
                ...
                for buffered in buffered_slices:
                    for candidate_id, candidate in enumerate(lock_candidates):
                        ...
                        sent = send_pulse_udp(
                            ...
                            tag_id=args.tag_id,
                            frequency_hz=report_frequency_hz(
                                args.center_freq, args.freq, candidate.freq_hz),
                ...
                for det in detections:
                    ...
                    if pulse_sock is not None:
                        ...
                        report_sent = send_pulse_udp(
                            ...
                            tag_id=args.tag_id,
                            frequency_hz=report_frequency_hz(
                                args.center_freq, args.freq, det.freq_hz),
