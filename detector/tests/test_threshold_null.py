...
def test_refining_on_strongest_noise_bin_does_not_lower_threshold():
    # The strongest bin is always a refinement candidate and on pure noise it
    # passes the train gate most of the time at K=5 (measured ~80 %; ~15 % at
    # K=20). That must not bias the threshold: dropping one bin's winning
    # windows from a max-over-all-bins null leaves it where it was.
    class Log:
        def __init__(self):
            self.record = None

        def emit(self, entry_type, human, **data):
            if entry_type == 'cycle_threshold':
                self.record = data
    ...
    for _ in range(30):
        ...
        power, n_time = compute_stft_power(iq, N_W, N_OL, NFFT, W=W)
        log = Log()
        fold_detect(power, N, PF, FS, NFFT, N_W, N_OL, SAMPLES,
                    {'rng': np.random.default_rng(0)},
                    fold_offsets=FOLD_OFFSETS, W=W, Wf=Wf, k_folds=K,
                    n_null_permutations=20, slog=log)
        if not log.record['refined']:
            continue
        noise = estimate_noise_power(power)
        hyps = build_hypothesis_indices(N_EXACT, K, n_time, N_B=None)
        unrefined, _, _, _ = permutation_null_threshold(
            power, noise, PF, 20, hypotheses=hyps, rng=np.random.default_rng(0))
        ratios.append(log.record['threshold'] / unrefined)
    assert len(ratios) >= 10, f'only {len(ratios)} refined slices; gate changed?'
    ratios = np.array(ratios)
    assert 0.95 < ratios.mean() < 1.05, ratios.mean()
    assert ratios.min() > 0.85, ratios.min()
