...
| `--center-freq` | `0.0` | Channel centre, MHz (frequency gate; pulse reports carry centre + detected offset) |
| `--freq` | `0` | Absolute tag frequency, Hz (reported in no-detection pulses and when `--center-freq` is unset; with `--center-freq` enables the ±2 kHz search gate) |
