...
class TestPulseOnRateSwitch:
    ...
    def test_post_switch_uses_secondary_rate(self):
        ...
        # Find pulse start times after the switch
        on = np.where(mask)[0]
        # Group into individual pulses (gaps > 0.5 * tip_b between groups)
        pulse_starts = [on[0] / fs]
        for i in range(1, len(on)):
            if (on[i] - on[i - 1]) / fs > 0.5 * tip_b:
                pulse_starts.append(on[i] / fs)

        # Keep only post-switch pulses
        post_starts = [p for p in pulse_starts if p >= 5.0]
        assert len(post_starts) >= 3, "Need at least 3 post-switch pulses"

        # Check inter-pulse intervals are close to tip_b
    ...
    def test_anchor_continuity(self):
        ...
        pulse_starts = [on[0] / fs]
        for i in range(1, len(on)):
            if (on[i] - on[i - 1]) / fs > 0.1:
                pulse_starts.append(on[i] / fs)

        # Last pre-switch pulse
        pre_pulses = [p for p in pulse_starts if p < switch_time]
        post_pulses = [p for p in pulse_starts if p >= switch_time]
        assert len(pre_pulses) >= 1 and len(post_pulses) >= 1

        last_pre = pre_pulses[-1]
        first_post = post_pulses[0]
        gap = first_post - last_pre
        # Gap should be tip_secondary (anchored to last pre-switch pulse)
    ...
    def test_anchor_on_boundary(self):
        ...
        # The pulse at switch_time must exist
        boundary_pulses = [p for p in pulse_starts if abs(p - switch_time) < 0.001]
        assert len(boundary_pulses) == 1, (
            f"Expected pulse at switch_time={switch_time}, "
            f"got starts near it: {boundary_pulses}")

        # Next pulse after boundary should be tip_secondary later
        post_pulses = [p for p in pulse_starts if p > switch_time + 0.1]
    ...
    def test_no_switch_fields_unchanged(self):
        """tip_secondary=None should behave identically to fixed rate."""
        tag_plain = TagSignal(tip=2.0, tp=0.015)
        tag_none = TagSignal(tip=2.0, tp=0.015, tip_secondary=None, switch_time=None)
        t = np.linspace(0, 10, 50000)
        np.testing.assert_array_equal(
            tag_plain.pulse_on(t),
            tag_none.pulse_on(t))

    def test_switch_time_zero(self):
        """switch_time=0 means all pulses at secondary rate."""
        tag = TagSignal(tip=2.0, tp=0.01, tip_secondary=1.0, switch_time=0.0)
        fs = 10000
        ...
        # All intervals should be ~1.0s
        for i in range(1, len(pulse_starts)):
            delta = pulse_starts[i] - pulse_starts[i - 1]
            assert abs(delta - 1.0) < 0.02
