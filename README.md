...
**Logs and post-flight analysis.** Each detection session gets a `~/Detectors-<UTC timestamp>/` directory (rotations use `~/Rotation-<timestamp>/heading-NNN/`). Python detectors write a machine-readable `detector_<tag>.jsonl` alongside their text log (schema: `shared/log_schema.py`). When a session stops, the controller runs `analyzer/post_flight_analysis.py` on that directory in the background and writes `analysis.md` there. If the GCS sets `dump_spectrogram`, each detection cycle also saves `tag<T>_cycle_NNNN_{power.npy,iq.npy,meta.json}` (~0.9 MB per cycle per tag at the defaults — ~0.6 MB power + ~0.3 MB IQ — so roughly 330 MB/hour) and the report embeds spectrogram PNGs (requires `matplotlib` in the venv).

The `dump_spectrogram` field was added to `StartDetectionInfo_t` and `StartRotationDetection_t` in `tunnel-protocol`; the controller rejects mismatched payload sizes, so TagTracker must be updated to the matching `TagTrackerTunnelProtocol` commit in lockstep.
